    ...
    <>
      ...
      <aside className="hidden lg:flex fixed left-0 top-0 z-40 h-screen w-64 flex-col border-r border-zinc-800/60 bg-zinc-950/80 backdrop-blur-xl">
        ...
        <div className="border-t border-zinc-800/60 px-4 py-4">
          <div className="flex items-center gap-3 rounded-lg px-2 py-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-teal-500 to-emerald-600 text-xs font-bold text-white shrink-0">
              {initials}
            </div>
            ...
            <button
              ...
              className="rounded-lg p-1.5 text-zinc-500 transition-colors hover:bg-zinc-800 hover:text-zinc-300 shrink-0"
              title="Sign out"
            >
      ...
      <header className="lg:hidden sticky top-0 z-30 flex w-full items-center justify-between border-b border-zinc-800/60 bg-zinc-950/80 px-4 py-3 backdrop-blur-xl">
        ...
        <div className="flex items-center gap-3">
          <div className="flex h-7 w-7 items-center justify-center rounded-full bg-gradient-to-br from-teal-500 to-emerald-600 text-[10px] font-bold text-white">
            {initials}
          </div>
          <button
            onClick={() => setShowLogoutDialog(true)}
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 flex items-center justify-around border-t border-zinc-800/60 bg-zinc-950/90 pb-safe pt-1 backdrop-blur-xl h-16 pointer-events-auto">
        {navItems.map((item) => {
          const isActive = pathname === item.href;
          return (
            <Link
      ...
      {/* Logout Confirmation Dialog - Moved outside aside to center properly on screen */}
      {showLogoutDialog && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 px-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl border border-zinc-800 bg-zinc-950 p-6 shadow-2xl">
            <h3 className="mb-2 text-lg font-semibold text-white">Sign Out</h3>
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 rounded-xl border border-zinc-800 bg-zinc-900 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-zinc-800"
              >
                Cancel
              </button>
              <button
                ...
                className="flex-1 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 px-4 py-2.5 text-sm font-medium text-white transition-all hover:opacity-90 shadow-lg shadow-red-900/20"
              >
                Sign Out
              </button>
            </div>
