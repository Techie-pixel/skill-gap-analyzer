...
def get_youtube_resources(skills: list[str]) -> list[YouTubeSkillResource]:
    """Fetch YouTube resources — uses YouTube Data API v3 with static fallback."""
    ...
    for skill in skills:
        # Try live YouTube API search first
        try:
            videos = search_youtube_videos(skill, max_results=3)
            if videos and videos[0].get("video_id"):
                links = [
                    YouTubeLink(title=v["title"], url=v["url"])
                    for v in videos
                ]
