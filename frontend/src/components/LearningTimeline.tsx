  ...
  return (
    <div className="flex h-[50vh] flex-col items-center justify-center space-y-6">
      <div className="relative flex items-center justify-center">
        ...
        <div className="absolute h-10 w-10 animate-pulse rounded-full bg-emerald-500/20 blur-xl" />
      </div>
      <div className="text-center">
        <h3 className="text-lg font-semibold text-white">Generating Roadmap...</h3>
        <p className="mt-2 text-sm text-zinc-400">AI is planning your personalized learning journey step-by-step.</p>
      </div>
    </div>
  );
}
...
export default function LearningTimeline() {
  ...
  const fetchRoadmap = async () => {
    ...
    try {
      ...
      setError("Could not load roadmap from backend. Please try again later.");
      setRoadmap(fallbackRoadmap);
      setTargetRole("Full Stack Developer");
    } finally {
      setIsLoading(false);
